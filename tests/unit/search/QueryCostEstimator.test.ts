...
describe('QueryCostEstimator', () => {
  ...
  describe('adaptive recordExecution / EWMA feedback', () => {
    it('falls back to the configured constant before any sample lands', () => {
      const e = new QueryCostEstimator();
      const before = e.estimateMethod('basic', 'q', 100);
      expect(e.getAdaptiveStats().sampleCounts.basic).toBeUndefined();
      expect(before.estimatedTimeMs).toBeGreaterThan(0);
    });

    it('first sample seeds the EWMA at the observed value', () => {
      const e = new QueryCostEstimator();
      e.recordExecution('basic', 100, 100); // 1ms per entity
      expect(e.getAdaptiveStats().baseTimes.basic).toBeCloseTo(1, 5);
      expect(e.getAdaptiveStats().sampleCounts.basic).toBe(1);
    });

    it('subsequent samples blend per the EWMA alpha', () => {
      const e = new QueryCostEstimator();
      e.recordExecution('basic', 100, 100); // observed = 1.0 ms/entity
      e.recordExecution('basic', 100, 200); // observed = 2.0 ms/entity
      // alpha = 0.2 → 0.2*2 + 0.8*1 = 1.2
      expect(e.getAdaptiveStats().baseTimes.basic).toBeCloseTo(1.2, 5);
    });

    it('recordExecution() with non-finite or non-positive inputs is a no-op', () => {
      const e = new QueryCostEstimator();
      e.recordExecution('basic', 0, 100);
      e.recordExecution('basic', 100, NaN);
      e.recordExecution('basic', 100, -5);
      expect(e.getAdaptiveStats().sampleCounts.basic).toBeUndefined();
    });

    it('adaptive value is preferred by getBaseTimeForMethod once seeded', () => {
      const e = new QueryCostEstimator();
      const baseline = e.estimateMethod('basic', 'q', 100).estimatedTimeMs;
      // Record an artificially-high observation; estimate should grow.
      e.recordExecution('basic', 100, baseline * 10);
      const adapted = e.estimateMethod('basic', 'q', 100).estimatedTimeMs;
      expect(adapted).toBeGreaterThan(baseline);
    });

    it('resetAdaptive() drops every sample so estimates revert to the configured constant', () => {
      const e = new QueryCostEstimator();
      e.recordExecution('basic', 100, 1000);
      e.resetAdaptive();
      expect(e.getAdaptiveStats().baseTimes.basic).toBeUndefined();
      expect(e.getAdaptiveStats().sampleCounts.basic).toBeUndefined();
    });
  });
});
