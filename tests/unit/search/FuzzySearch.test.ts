...
describe('FuzzySearch - Sprint 14 Extended Tests', () => {
  ...
  describe('Bloom pre-screener integration', () => {
    // Local fixture so the tests are self-contained — the outer
    // Sprint-14 beforeEach only creates an empty storage.
    beforeEach(async () => {
      const entityManager = new EntityManager(storage);
      await entityManager.createEntities([
    ...
    it('hasBloomPreScreener reflects attach/detach state', () => {
      const fuzzy = new FuzzySearch(storage, { useWorkerPool: false });
      expect(fuzzy.hasBloomPreScreener()).toBe(false);
      fuzzy.setBloomPreScreener(null);
      expect(fuzzy.hasBloomPreScreener()).toBe(false);
    });

    it('attaching a screener preserves recall — every baseline match also appears with the screener attached', async () => {
      const { BloomPreScreener } = await import('../../../src/search/BloomPreScreener.js');
      const fuzzy = new FuzzySearch(storage, { useWorkerPool: false });

      const baseline = await fuzzy.fuzzySearch('Alise', 0.7);

      const screener = new BloomPreScreener(storage);
      await screener.build();
      fuzzy.setBloomPreScreener(screener);
      const screened = await fuzzy.fuzzySearch('Alise', 0.7);

      const baselineNames = new Set(baseline.entities.map((e) => e.name));
      const screenedNames = new Set(screened.entities.map((e) => e.name));
      for (const name of baselineNames) {
        expect(screenedNames.has(name)).toBe(true);
      }
    });

    it('detaching the screener returns to baseline behaviour', async () => {
      const { BloomPreScreener } = await import('../../../src/search/BloomPreScreener.js');
      const fuzzy = new FuzzySearch(storage, { useWorkerPool: false });
      const screener = new BloomPreScreener(storage);
      await screener.build();
      fuzzy.setBloomPreScreener(screener);
      expect(fuzzy.hasBloomPreScreener()).toBe(true);
      fuzzy.setBloomPreScreener(null);
      expect(fuzzy.hasBloomPreScreener()).toBe(false);
      const result = await fuzzy.fuzzySearch('Alice');
