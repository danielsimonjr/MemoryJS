...
describe('ConflictResolver', () => {
  ...
  describe('resolveConflict', () => {
    ...
    it('should resolve using trust_level strategy — highest trust wins', () => {
      const memories = [
        createMockMemory('inferred_mem', 'agent_1', ['Data'], {
          source: {
            agentId: 'agent_1',
        ...
        createMockMemory('ground_truth_mem', 'agent_2', ['Data'], {
          source: {
            agentId: 'agent_2',
            timestamp: new Date().toISOString(),
            method: 'told',
      ...
      const conflict = {
        primaryMemory: 'inferred_mem',
        conflictingMemories: ['ground_truth_mem'],
        detectionMethod: 'similarity' as const,
        suggestedStrategy: 'trust_level' as const,
        detectedAt: new Date().toISOString(),
      };

      const result = resolver.resolveConflict(conflict, memories, agents, 'trust_level');

      expect(result.resolvedMemory.name).toBe('ground_truth_mem');
      expect(result.strategy).toBe('trust_level');
    });

    it('should respect explicit `source.trustLevel` over inferred', () => {
      const memories = [
        createMockMemory('explicit_ground', 'agent_1', ['Data'], {
          source: {
            agentId: 'agent_1',
            timestamp: new Date().toISOString(),
            method: 'inferred',
            reliability: 0.1,
            trustLevel: 'ground-truth',
          },
        }),
        createMockMemory('inferred_verified', 'agent_2', ['Data'], {
          source: {
            agentId: 'agent_2',
            timestamp: new Date().toISOString(),
            method: 'observed',
      ...
      const conflict = {
        primaryMemory: 'explicit_ground',
        conflictingMemories: ['inferred_verified'],
        detectionMethod: 'similarity' as const,
        suggestedStrategy: 'trust_level' as const,
        detectedAt: new Date().toISOString(),
      };
      ...
      expect(result.resolvedMemory.name).toBe('explicit_ground');
    });

    it('should break ties at the same trust tier by recency', () => {
      const oldDate = new Date('2024-01-01').toISOString();
      const newDate = new Date('2024-12-01').toISOString();

      const memories = [
        createMockMemory('older_verified', 'agent_1', ['Data'], {
        ...
        createMockMemory('newer_verified', 'agent_2', ['Data'], {
      ...
      const conflict = {
        primaryMemory: 'older_verified',
        conflictingMemories: ['newer_verified'],
        detectionMethod: 'similarity' as const,
        suggestedStrategy: 'trust_level' as const,
        detectedAt: new Date().toISOString(),
      };
      ...
      expect(result.resolvedMemory.name).toBe('newer_verified');
    });

    it('should fall back to `unverified` when `source` is missing', () => {
      const memories = [
        createMockMemory('no_source', 'agent_1', ['Data'], {}),
        createMockMemory('with_inferred', 'agent_2', ['Data'], {
          source: {
            agentId: 'agent_2',
            timestamp: new Date().toISOString(),
            method: 'inferred',
      ...
      const conflict = {
        primaryMemory: 'no_source',
        conflictingMemories: ['with_inferred'],
        detectionMethod: 'similarity' as const,
        suggestedStrategy: 'trust_level' as const,
        detectedAt: new Date().toISOString(),
      };
      ...
      // 'inferred' > 'unverified'
      expect(result.resolvedMemory.name).toBe('with_inferred');
    });
