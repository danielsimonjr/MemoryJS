...
describe('CollaborativeSynthesis', () => {
  ...
  describe('η.5.5.a resolveConflicts', () => {
    ...
    it('trust_level picks the candidate with the highest TrustLevel', async () => {
      const aliceA = makeAgentEntity({
        name: 'Alice-v1', entityType: 'person',
        agentId: 'agent-a', rootEntityName: 'Alice',
        confidence: 0.6, lastModified: '2025-06-01T00:00:00Z',
        source: {
          agentId: 'agent-a', timestamp: '2025-06-01T00:00:00Z',
          method: 'user_provided', reliability: 0.5,
          trustLevel: 'inferred',
        },
      });
      const aliceB = makeAgentEntity({
        name: 'Alice-v2', entityType: 'person',
        agentId: 'agent-b', rootEntityName: 'Alice',
        confidence: 0.6, lastModified: '2024-01-01T00:00:00Z', // older — proves trust trumps recency
        source: {
          agentId: 'agent-b', timestamp: '2024-01-01T00:00:00Z',
          method: 'user_provided', reliability: 0.9,
          trustLevel: 'verified',
        },
      });
      const storage = makeMockStorage({
      ...
      const result = await synth.synthesize(SEED);
      const winners = synth.resolveConflicts(result, { strategy: 'trust_level' });
      expect(winners.get('Alice')?.agentId).toBe('agent-b');
    });

    it('trust_level falls back to most-recent for same-TrustLevel candidates', async () => {
      const aliceA = makeAgentEntity({
        name: 'Alice-v1', entityType: 'person',
        agentId: 'agent-a', rootEntityName: 'Alice',
        confidence: 0.6, lastModified: '2024-01-01T00:00:00Z',
        source: {
          agentId: 'agent-a', timestamp: '2024-01-01T00:00:00Z',
          method: 'user_provided', reliability: 0.9,
          trustLevel: 'verified',
        },
      });
      const aliceB = makeAgentEntity({
        ...
        confidence: 0.6, lastModified: '2025-06-01T00:00:00Z',
        source: {
          agentId: 'agent-b', timestamp: '2025-06-01T00:00:00Z',
          method: 'user_provided', reliability: 0.9,
          trustLevel: 'verified',
        },
      });
      ...
      const result = await synth.synthesize(SEED);
      const winners = synth.resolveConflicts(result, { strategy: 'trust_level' });
      expect(winners.get('Alice')?.agentId).toBe('agent-b'); // later lastModified wins the tie
    });
