...
> - **Gap 3 (blob decomposition):** ✅ `ProcedureStore` decomposed — steps are `procedure-step` entities with `has_step`/`precedes`/`has_fallback` relations; legacy sentinel entities auto-migrate on load. Remaining blob users (`WorkThreadManager`, `SessionCheckpoint`, `ProfileManager`) are follow-ups.
