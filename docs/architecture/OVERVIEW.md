...
| Stable identity | `Entity.id` (UUID, assigned at creation, preserved across renames on both backends) + `EntityManager.renameEntity()` — atomically rewrites relations, hierarchy, version-chain fields, and `RefIndex` aliases; emits `entity:renamed` → `entity:deleted` → `entity:created` |
| Storage backends | JSONL (human-readable) or SQLite (FTS5 + BM25); pluggable `IMemoryBackend` for the memory engine — both now expose a `GraphEventEmitter` (`storage.events`) with full event parity |
...
| Agent Memory System | Sessions, working memory, episodic memory, decay, multi-agent visibility, role profiles, entropy filtering, recursive consolidation, Procedural Graph |
...
| Memory theory | Procedural memory, Procedural Graph (`ProcedureManager` + `createProceduralGraph` / `ProceduralGraphManager`), active retrieval, causal reasoning, world-model orchestrator |
...
│  Search:  Ranked / Semantic / Temporal / HybridSearch /│
...
│  Theory:  ProcedureManager / createProceduralGraph /   │
│           ProceduralGraphManager / CausalReasoner /    │
│           WorldModelManager                            │
...
│    read pool, PartialIndexAdvisor, GraphEventEmitter   │
│    parity with GraphStorage)                           │
└────────────────────────────────────────────────────────┘
```
...
interface Entity {
  // Core
  name: string;           // Unique identifier — the public key
  id?: string;             // Stable opaque UUID, assigned at creation,
                           // preserved across renames on both backends.
                           // Forward-compat for v2 reference migration;
                           // `name` remains the reference key today.
...
│   ├── events/       # EventManager (event reification)
│   ├── procedural/   # ProcedureManager + StepSequencer
│   │   └── graph/    # ProceduralGraph + Manager + Evolution +
│   │                 #   Session + JSONL/SQLite/memory backings
│   ├── reconstruction/ # ReconstructiveMemory (Cue–Tag–Content)
...
│   │                 #   managers, GraphStorage / SQLiteStorage /
│   │                 #   PostgreSQLStorage, graph traversal
...
7. **Event-driven cache invalidation**: `GraphEventEmitter` triggers TF-IDF index (and `GraphRankPrior`) updates on entity changes — both storage backends now emit the same event surface, so derived views stay correct regardless of backend.
...
Generated 2026-09-10 from `dependency-summary.compact.json` (package 4.0.0).
Regenerate: `bun run tools:deps` · Test coverage: `bun run tools:deps:full`

| Claim | Value | Source |
|---|---|---|
| sourceFiles | 291 (all reachable, 0 dormant) | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| totalLinesOfCode | 100890 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| totalExports | 1960 (1287 re-exports) | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| classes | 231 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| interfaces | 617 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| functions | 312 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| typeGuards | 29 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| enums | 4 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| constants | 118 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| typeOnlyImports | 473 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| runtimeCircularDeps | 0 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| typeOnlyCircularDeps | 8 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| modules | adapters 7, agent 105, cli 31, core 25, features 18, entry 1, search 51, security 5, root 1, types 12, utils 34, workers 1 | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| entryRoots | `src/cli/index.ts`, `src/index.ts`, `src/sqlite.ts`, `src/workers/levenshteinWorker.ts` | dependency-summary.compact.json / DEPENDENCY_GRAPH.md |
| testFiles | 361 | TEST_COVERAGE.md |
| sourcesWithDirectImportTests | 280/291 (96.2%) | TEST_COVERAGE.md |
