...
> See [docs/architecture/DEPENDENCY_GRAPH.md](../architecture/DEPENDENCY_GRAPH.md)
> (auto-generated by `tools/create-dependency-graph`) and
> [docs/architecture/COMPONENTS.md](../architecture/COMPONENTS.md).
> Patterns documented below remain accurate; the new sub-modules under
> `agent/` (causal, procedural, retrieval, world, rbac, collaboration) and
> the new `security/` module follow the same wrap-and-extend pattern
> documented in [ADR-011](../development/ARCHITECTURE_DECISIONS.md).
