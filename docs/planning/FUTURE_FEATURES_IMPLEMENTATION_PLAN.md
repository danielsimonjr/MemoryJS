...
| ☑ | 24 | **§15.8 Public API tiering** | 1–2 wk | ✅ Shipped in Phase 5. Tagged 19 Phase 0–4 modules with `@public` / `@experimental` JSDoc + new "API Stability Tiers" policy section in `CLAUDE.md`. Took the alt path (tag only post-Phase-0 symbols, leave pre-existing modules at the implicit `@public` level) so this is not a SemVer-breaking change. A full audit + per-symbol tiering of pre-Phase-0 modules is deferred to a v2.0.0 cut. |
...
| ☑ | 29 | **§15.1 Split god-objects (first pass on `IOManager.ts`)** | 2 wk | ✅ Shipped in Phase 5 (first pass). Extracted backup lifecycle from `IOManager` into `src/features/BackupManager.ts` (~313 LOC: `create` / `list` / `restore` / `delete` / `cleanOld` + `getDir`). `IOManager` keeps a private `BackupManager` instance and delegates the 6 backup methods to it; pre-extraction public API unchanged. New `IOManager.backupManager` getter exposes the smaller surface. 15 unit tests covering extraction wiring, the full lifecycle, and delegation parity. The remaining `IOManager` sub-modules (export, import, ingest, visualize) will be extracted in follow-up commits. |

Phase 2 result: all 12 items shipped (steps 24, 29 closed in Phase 5). New env vars: `MEMORY_SQLITE_AUTO_INDEX`, `MEMORY_SYNONYM_EXPANSION`, `MEMORY_CACHE_BUDGET_ENTRIES`. Three files of new infrastructure (`MaterializedViews`, `BloomPreScreener`, `CachePressureCoordinator`, `PartialIndexAdvisor`) shipped with smoke-test coverage but not yet wired into the search/cache hot paths — caller integration is a follow-up.
...
| ☑ | 52 | **§13.4 SPARQL** ✅ Shipped in Phase 6. New `src/search/SPARQL.ts`. Minimal SPARQL 1.1 SELECT subset (PREFIX, SELECT [DISTINCT], WHERE { triples }, FILTER, LIMIT/OFFSET). Hand-rolled tokenizer disambiguates `<` (operator) from `<iri>` based on next-char. Patterns are reordered by selectivity (bound terms first); `maxSolutions` and `maxLikePatternLength` caps protect against CPU-DoS queries. `runSparql(query, graph, limits?)` is the convenience entry. OPTIONAL/UNION/MINUS/BIND/GROUP BY/property paths/CONSTRUCT/ASK/DESCRIBE/Update remain out of scope — callers needing those should defer to a real engine. |
...
| ☐ | # | Task | Acceptance |
|---|---|------|------------|
| ☑ | 59 | **7.1 Segment storage interface** | ✅ `src/core/segments/ISegmentStorage.ts` with `Segment`, `SegmentId`, `SegmentRouter`, `ISegmentStorage`. `FnvSegmentRouter` (FNV-1a 32-bit modulo `segmentCount`), pure `splitGraphIntoSegments` / `mergeSegmentsIntoGraph` helpers, `InMemorySegmentStorage` reference impl with ownership validation. 20 tests. |
| ☑ | 60 | **7.2 FileSegmentStorage backend** | ✅ `src/core/segments/FileSegmentStorage.ts`. Per-segment atomic writes via temp+fsync+rename (matches `GraphStorage.durableWriteFile` with the Windows EPERM fallback). **Manifest-based crash-atomic `saveAll`** (review #4): writes a `segments/_manifest.json` sidecar listing the rename moves, then a crash mid-rename is recovered forward on the next `loadAll`. Per-line malformed-JSONL tolerance (review #6). 25 tests + 3 manifest-recovery + 1 malformed-line. |
| ☑ | 61 | **7.3 Cross-segment relation routing** | ✅ Shipped alongside task 60. `findOutgoingRelations(name)` reads one segment (O(segment-size)); `findIncomingRelations(name)` reads all segments (O(graph-size)) and documents the asymmetry. |
| ☑ | 62 | **7.4 Wire `GraphStorage` to segments** | ✅ New `segmentStorage` field on `GraphStorage`, populated from `MEMORY_STORAGE_SEGMENT_COUNT` env. **Strict regex parsing** rejects floats / exponents / hex / leading-zero / signs; upper bound `MAX_SEGMENT_COUNT = 1024` (review #8). Append paths (`appendEntity`, `appendRelation`, `updateEntity`) fall back to a full saveAll via `appendViaSegmentSave`. Reload-failure path catches the secondary error and surfaces both as an aggregated "desynced state" message (review #1). `pendingAppends` reset (review #2); `clearIndexes()` defense-in-depth (review #3); deep-clone `previous` snapshot for change events (review #9). 12 wiring tests + 5 env-parsing + 1 reload-failure. |
| ☑ | 63 | **7.5 Migration tool** | ✅ `tools/segment-jsonl/`. CLI for `split` / `merge` over an existing JSONL. Strict numeric parsing of `--segments` (review #12). Overwrite-protection via `--force` flag — refuses to clobber existing segment files / output paths by default (review #13). 20 tool tests + 3 review-fix tests covering the bidirectional round-trip. |
...
| ☐ | # | Task | Acceptance |
|---|---|------|------------|
| ☑ | 64 | **8.1 IColumnStore interface** | ✅ `src/core/columns/IColumnStore.ts`. Generic `IColumnStore<T>` with `get / has / put / delete / batchPut / keys / entries / size / clear / reload`. `ObservationColumn = string[]` named alias. `InMemoryColumnStore<T>` reference impl distinguishes "absent key" from "explicit empty value" via `has()`. 14 tests. |
| ☑ | 65 | **8.2 JsonlColumnStore backend** | ✅ `src/core/columns/JsonlColumnStore.ts`. Whole-file rewrite on every mutation via temp+fsync+rename with Windows EPERM fallback. **Snapshot-restore rollback** on flush failure (review #1) — `put` / `delete` / `batchPut` / `clear` all capture pre-mutation cache state and restore it if the disk flush throws, satisfying the `IColumnStore.batchPut` "all-or-nothing" contract. Per-line tolerance on load (skips malformed lines + warns). New `reload()` method (review #4) drops the cache so external sidecar edits (migration tool) become visible. 16 + 5 review-fix rollback tests + 2 reload tests. |
| ☑ | 66 | **8.3 Read-path integration** | ✅ `ObservationManager.getObservationsFor(name)` — reads column store first when attached, falls back to inline via `storage.getEntityByName`. Returns defensive copy. Pre-Phase-8 storage format works unchanged through the fallback. Mixed-format graphs (some entities migrated, others not) tested. |
| ☑ | 67 | **8.4 Write-path integration** | ✅ `addObservations` / `deleteObservations` shadow-write the column store after the existing `saveGraph` succeeds. **Event-driven fan-out via `GraphEventEmitter`** (review #3): `setColumnStore` also subscribes to `entity:created` / `entity:updated` / `entity:deleted` / `graph:saved` so observation writes through other paths (`EntityManager.createEntities` with non-empty observations, `updateEntity` with `observations` patch, the v1.8.0 supersede branch, bulk imports) are captured too. `entity:deleted` drops the column entry — addresses review #2 (ghost data for deleted entities). `graph:saved` resyncs from storage since bulk-save paths don't emit per-entity events. Shadow-write failures log but never reject the calling write (inline state is already durable via saveGraph). |
| ☑ | 68 | **8.5 Migration helper** | ✅ `tools/observations-to-columns/`. CLI for `extract` / `reinline`, exports `runExtract` / `runReinline` / `parseArgs` for programmatic use. Strict `--segments`-style parsing rejects floats / exponents. `--force` flag protects against clobbering. **Double-extract refusal** (review #6) — refuses to overwrite a populated sidecar with an empty one (the "already extracted, run reinline instead" footgun). **Orphan recovery** (review #7) — sidecar entries with no matching entity get written to `<output>.orphans.jsonl` + a loud `console.warn` so wrong-pairing accidents are recoverable. **ENOENT alignment** (review #8) — `readColumnSidecar` returns an empty map for an absent sidecar instead of letting the raw ENOENT bubble up. 29 tool tests. |
...
| 2026-05-11 | 8 | **Phase 8 — Columnar observation storage** (tasks 64, 65, 66, 67, 68 — all five). Second multi-month feature. Same orchestration pattern as Phase 7 (interface in main, parallel worktree agents on tasks 65 + 68, orchestrator-written wiring for tasks 66+67, review on cumulative diff, close-out). **Operational note:** worktree harness cleanup deleted agent B's files from main's working tree between reconciliation commits — recovered via `git checkout <prior-sha> -- <path>` in a WIP-3 commit. Future phases should reconcile-and-commit immediately after each agent completes rather than batching, to avoid the cleanup window. | ✅ Shipped. One review round → 15 findings (12 substantive + 2 nits + 1 confirmation); 11 substantive fixes applied: **#1+#10 snapshot-restore rollback** in `JsonlColumnStore.put/delete/batchPut/clear` (restores the `IColumnStore.batchPut` "all-or-nothing" JSDoc contract); **#2+#3+#11+#12 event-driven fan-out** — `setColumnStore` subscribes to `entity:created` / `entity:updated` / `entity:deleted` / `graph:saved` on `GraphEventEmitter`, capturing observation writes from `EntityManager.createEntities` / `updateEntity` / supersede branch / bulk imports that bypass `addObservations`; `entity:deleted` cleans up the column entry (kills ghost data); `graph:saved` resyncs from storage for paths that don't emit per-entity events; **#4 `reload()` method** on `IColumnStore` so external sidecar edits become visible to long-running processes; **#5 doc** — explicit JSDoc that `observationColumnStore` is cached at first access and not runtime-flippable (matches Phase 7 precedent); **#6 double-extract refusal** in migration tool — refuses to clobber a populated sidecar with an empty one (operator footgun); **#7 orphan recovery** — wrong-pairing accidents write a `<output>.orphans.jsonl` for data recovery + a loud `console.warn`; **#8 ENOENT alignment** — `readColumnSidecar` returns empty map for absent files instead of bubbling raw ENOENT; **#14 sidecar path renamed** from `<basename>.observations.jsonl` to `<basename>-observations.jsonl` (hyphen matches existing `<basename>-saved-searches.jsonl` / `-tag-aliases.jsonl` / `-ref-index.jsonl` convention). **#9 pre-existing `addObservations` race deferred** — concurrent calls bypass the graph mutex and can interleave; Phase 8 makes it observable through the column store but doesn't fix it; documented in CHANGELOG. **#13 nit** — new `MEMORY_OBSERVATIONS_COLUMNAR` env var added to CLAUDE.md matrix. Tests: 72 new (14 interface + 16 file backend + 29 migration tool + 13 wiring + 12 review-fix regression: 5 rollback × 2 reload × 1 deleted-ghost × 3 bypass-paths × 1 hyphen-path). Final tally: `npm run lint` exits 0; `npm run typecheck` clean; `SKIP_BENCHMARKS=true vitest run` passes 6794/6801 (was 6710 at end of Phase 7, +84); `npm audit` reports 0 vulnerabilities. | `5c62046` (task 64) + `2019439` (task 65 + harness-leaked task 68) + `2b94279` (wiring tasks 66+67) + `dbd5f2c` (task 68 restore) + close-out commit |
| 2026-05-11 | 7 | **Phase 7 — JSONL segment files** (tasks 59, 60, 61, 62, 63 — all five). First multi-month engineering feature from the Phase 7-11 agent-driven breakdown. **Orchestration pattern:** I (orchestrator) wrote task 59 (the interface contract) myself, then dispatched two parallel agents in git worktrees (Agent A: tasks 60+61 `FileSegmentStorage`; Agent B: task 63 migration tool) to work simultaneously, reconciled their output by copying the new files into main (foundation files verified byte-identical via `diff -q`), then wrote task 62 (`GraphStorage` wiring) myself in main. Saved ~10 min vs sequential. | ✅ Shipped. One review round (general-purpose subagent on cumulative diff → 13 findings: 3 critical + 9 substantive + 1 nit; 9 substantive fixes applied: #1 `appendViaSegmentSave` reload-failure surfaces both errors as aggregated "desynced" message, #2 `pendingAppends` reset in segment-mode append, #3 `clearIndexes()` defense-in-depth before reload, **#4 manifest-based crash-atomic `saveAll`** — new `segments/_manifest.json` sidecar drives forward-recovery in `loadAll`, replacing the misleading "two-phase staging" JSDoc, #5 tmp-file cleanup on partial-rename failure, #6 per-line malformed-JSONL tolerance in `parseSegmentFile`, #8 strict-decimal regex parsing of `MEMORY_STORAGE_SEGMENT_COUNT` rejecting floats / exponents / hex / leading-zeros / signs, plus `MAX_SEGMENT_COUNT=1024` upper bound, #9 deep-clone `previous` snapshot in segment-mode `updateEntity` to survive `Object.assign` mutation, #12 strict `--segments` parsing in migration tool, #13 `--force` flag for overwrite protection in both split/merge). Tests: 90 new (20 interface + 25 file backend + 20 migration tool + 12 wiring + 13 review-fix regression tests). Also fixed a stale CLAUDE.md note ("No lint script configured") that tripped both parallel agents — the project does have `"lint": "eslint src"`. Final tally: `npm run lint` exits 0; `npm run typecheck` clean; `SKIP_BENCHMARKS=true vitest run` passes 6710/6717 (was 6620 pre-Phase-7, +90); `npm audit` reports 0 vulnerabilities. | `c94b929` (task 59) + `722563e` (WIP: tasks 60–63) + `7b72c0c` (CLAUDE.md fix) + close-out commit |
| 2026-05-11 | 6 | **3 deferral closeouts** (Phase 3 step 36 `EntityProxy`, Phase 3 step 40 JSONL WAL, Phase 5 step 52 SPARQL minimal subset) — all three tractable without new external deps. | ✅ Shipped. Step 52: ~440 LOC hand-rolled tokenizer + recursive-descent parser + brute-force triple-matching evaluator over the existing RDF triple view (`IOManager.exportAsTurtle` convention). Step 36: ~170 LOC `EntityProxy` / `EntityProxyFactory` — eager `(name, entityType)`, deferred fields via a single `getEntityByName` on first access; arrays frozen on hand-out so callers can't corrupt the storage cache. Step 40: ~250 LOC append-only WAL with synchronous fsync ordering, POSIX directory fsync on first write, malformed-line tolerance only at the tail (mid-log corruption throws unless `tolerateGaps: true`). One review round (13 findings → 6 substantive fixes: #1 SPARQL `<`/`<=` operator tokens were unreachable behind the IRI scanner — disambiguated by peeking at next char; #2 SPARQL FILTER LIKE compiles regex once per query + length cap to prevent ReDoS; #3 SPARQL BGP join now has a `maxSolutions` budget + selectivity-based pattern reorder; #7 WAL fsyncs the parent directory on first write so the new dirent is durable on POSIX; #8 WAL replaced `require('fs')` (broken in native ESM) with a top-level `statSync` import; #9 WAL `replay()` throws on mid-log malformed lines by default; #11 EntityProxy freezes returned `observations` / `tags` arrays so caller `.push()` fails fast instead of corrupting the storage-layer cache). Tests: 72 new (SPARQL x32, EntityProxy x16, WAL x24). Final tally: `npm run lint` exits 0; `npm run typecheck` clean; `SKIP_BENCHMARKS=true vitest run` passes 6620/6627; `npm audit` reports 0 vulnerabilities. | `50c97db` (impl WIP) + close-out commit |
...
*Last updated: 2026-05-11 — Phases 0–8 shipped (see ledger). Phase 8 result: all 5 tasks (64–68) of the second multi-month feature (columnar observation storage) shipped. Orchestration pattern stable — interface task first, parallel agents in worktrees for backend + migration tool, orchestrator-written event-driven wiring, review + close-out. Phase 8 added 72 tests + applied 11 substantive review fixes (snapshot-restore rollback, event-driven fan-out via `GraphEventEmitter`, reload primitive, double-extract refusal, orphan recovery in migration tool, sidecar path rename, etc.). One pre-existing concurrency issue (#9: `addObservations` doesn't acquire graph mutex) deferred. Remaining deferrals: Phase 3 37/39/41 (now Phases 9/10/11 — tiered index, LZ4, mmap), Phase 4 42/43/45 (dep approval), Phase 5 55–58 (strategy decisions / dep approval). `vitest run` passes 6794/6801.*
