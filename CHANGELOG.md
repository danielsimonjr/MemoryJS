...
- The code-docs gate (`code_docs.py check src`) passes: 1326 of 1326 exported symbols have a doc comment. 255 issues are fixed: missing comments, comments without a summary line, and `@param` names that do not match the signature.
- Three source files change so that the code-docs parser can read them, with no runtime change. `CueTagContentGraph.ts` writes its NUL separator as the `'\u0000'` escape, not a raw byte. `ManagerContext.queryNaturalLanguage` and `SearchManager.getSearchCostEstimates` use named type imports in place of inline `import()` types.
...
- **REST request budgets.** With `auth` set, a default pre-authentication limiter (60 failures per client address, 1 per second refill) returns 429 to invalid keys from an address whose budget is empty; only failed authentications consume it, and no body is read for them. `RestRouterOptions.rateLimiter` adds an optional per-key limiter. `serve()` ends a slow JSON body with 408 after `bodyTimeoutMs` (default 10 s). `dispatch()` applies `maxBodyBytes` itself. `RestRouterOptions.limits` bounds the search query length, name length, observation count and observation length.
...
- **Default-open keys are visible.** A key without `projectIds` can access ALL projects. When a `RestRouter` with `auth` starts, it logs one warning per process with the number of unscoped, non-revoked keys (for example `3 API keys have no projectIds and can access all projects`). The warning contains no key material. New `APIKeyStore.unscopedKeyCount()` and `ApiKeyAuthMiddleware.unscopedKeyCount()`. ADR-012 in `docs/development/ARCHITECTURE_DECISIONS.md` records the tenancy model and a proposal (needs owner approval) to require explicit `projectIds` in the next major version.
- **Review hardening.** The failure budget no longer blocks valid keys: a valid key from an exhausted address still passes, and requests without `clientAddress` skip the budget. IPv6 clients share one bucket per /64. IPv4-mapped IPv6 spellings share the IPv4 bucket, and invalid addresses share one bucket. A scoped `DELETE` returns 409 when a relation (read through the relation index) connects the entity to an existing entity the key cannot see. A scoped search with at most 8 projects runs once per project, so the search result cap is not consumed by other tenants' matches; above 8 it runs one search and filters. A scoped create that a concurrent create pre-empts returns 409.
- **Copilot review fixes.** `RateLimiter.peek()` no longer refreshes a bucket, so expiry order stays consistent. `APIKeyStore` rejects sparse `projectIds` arrays. `bodyTimeoutMs` must be at most 2 147 483 647 ms (the Node timer range). The default `limits` match the entity schema (name 500, `entityType` 100 as the new `maxEntityTypeLength`, observation 5000 characters), empty or whitespace-only fields return 400, a `ValidationError` from `createEntities` returns `400 Invalid entity` instead of 500, and `GET`/`DELETE /entities/:name` bound the path name.
